import { HtmlCodeService } from "chrome-extension://cibffnhhlfippmhdmdkcfecncoaegdkh/editor/js/service/html-code-service.js";
var intro = introJs();
const elementos = [];
var indicesAnterior = [];

const modal = document.getElementById("modalSummerNote");
const btn = document.getElementsByClassName("button")[0];

$("#modalSummerNote").on("hidden.bs.modal", function (e) {
  //  alert("Modal foi fechado!");
});

function onMouseHoverElement(tabAtiva, event) {
  if (
    !$(event.target.parentElement).hasClass("tab-pin") &&
    !$(event.target.parentElement).closest("#erp-tabs").length &&
    !$(event.target.parentElement).closest('[ng-bind-html="tab.content"]')
      .length &&
    !$(event.target.parentElement).closest("#header").length &&
    !$(event.target.parentElement).closest(".navbar.fixed").length &&
    !$(event.target.parentElement).closest(".tab.new").length &&
    !event.target.closest("ul.tabs")
    //  "asd" === "asd"
  ) {
    if (isInspecionarPausado()) {
      if (event === undefined) event = window.event;
      const target = "target" in event ? event.target : event.srcElement;
      const tooltip = document.createElement("span");
      tooltip.setAttribute("locator-data-tooltip", target.tagName);
      tooltip.setAttribute(
        "data-html2canvas-ignore",
        "data-html2canvas-ignore"
      );

      target.appendChild(tooltip);
      markElementTemp(target);
      elementoSelecionado(target);

      target.addEventListener("mouseout", function () {
        unMarkElement(target, true);

        if (tooltip.parentNode) {
          target.removeChild(tooltip);
        }
      });
    }
  }
}

function isInspecionarPausado() {
  return (
    document.getElementById("iniciar-fluxo-help").innerText === "Iniciado..."
  );
}

function getXPath(elemento) {
  delete elemento.__jsaction;
  if (elemento.id !== undefined && elemento.id !== "") {
    var xpathWithId = '//*[@id="' + elemento.id + '"]';
    return xpathWithId;
  }
  if (elemento === document.body && elemento.id !== undefined)
    return elemento.tagName.toLowerCase();
  var ix = 0;

  if (elemento.id !== undefined) {
    var siblings = elemento.parentNode.childNodes;

    for (var i = 0; i < siblings.length; i++) {
      var sibling = siblings[i];
      if (sibling === elemento) {
        var selector =
          getXPath(elemento.parentNode) +
          "/" +
          elemento.tagName.toLowerCase() +
          "[" +
          (ix + 1) +
          "]";

        return selector.replace("undefined/", "");
      }

      if (sibling.nodeType === 1 && sibling.tagName === elemento.tagName) {
        ix++;
      }
    }
  }
}

function getCssPath(elemento) {
  delete elemento.__jsaction;
  var path = [];
  while (elemento.nodeType === Node.ELEMENT_NODE) {
    var selector = elemento.nodeName.toLowerCase();
    if (elemento.id) {
      selector += "#" + elemento.id;
      path.unshift(selector);
      break;
    } else {
      var sib = elemento,
        nth = 1;
      while ((sib = sib.previousElementSibling)) {
        if (sib.nodeName.toLowerCase() == selector) nth++;
      }
      if (nth != 1) selector += ":nth-of-type(" + nth + ")";
    }
    path.unshift(selector);
    elemento = elemento.parentNode;
  }
  return path.join(" > ");
}

function getElementAttributes(element) {
  var attributes = [];
  var elementAttributeNames = element.getAttributeNames();
  for (var index = 0; index < elementAttributeNames.length; index++) {
    if (
      elementAttributeNames[index] == "class" &&
      element.getAttribute(elementAttributeNames[index]) == ""
    ) {
      continue;
    }
    attributes.push(
      new ElementAttribute(
        elementAttributeNames[index],
        element.getAttribute(elementAttributeNames[index])
      )
    );
  }
  return attributes;
}

function marcaAlvo(elemento) {
  if (elemento) {
    if (elemento.elementLocator.cssSelector) {
      showElementOnPage(elemento.elementLocator);
    } else if (elemento.elementLocator.length) {
      if (currentElement) {
        markElementSuccess(currentElement);
        unMarkElement(currentElement);
        currentElement = null;
      }
    }
  } else {
    //verificar unimark
    unMarkElement(elemento.elementLocator, true);
    // unMarkAllMarks();
  }
}

async function obterPassosPorTabMenu(tabMenu) {
  const htmlCodeTab = await HtmlCodeService.obterPassosPorTabMenu(tabMenu);
  const selectElement = document.querySelector("#fluxos");
  debugger;
  $("#fluxos").empty();
  if (htmlCodeTab == "") {
    const option = document.createElement("option");
    option.text = "Não há passos";
    selectElement.add(option);
    return;
  }

  htmlCodeTab.forEach((htmlCode) => {
    const option = document.createElement("option");
    option.value = htmlCode.id;
    option.text = htmlCode.titulo;
    selectElement.add(option);
  });
}

const btnHtmlCodeEditar = document.querySelector("#btnHtmlCodeEditar");
btnHtmlCodeEditar.addEventListener("click", async () => {
  const selectElement = document.querySelector("#fluxos");
  const htmlCode = await HtmlCodeService.obterHtmlCodePorId(
    selectElement.value
  );

  htmlCodeEditar(htmlCode);
});

function htmlCodeEditar(htmlCode) {
  openModal();
  summerNote();
  $("#elemento").val(htmlCode[0].id);
  $("#summernote").summernote("code", htmlCode[0].code);
  $("#input-field-titulo").val(htmlCode[0].titulo);
  $("#ordenacao-modal").val(parseInt(htmlCode[0].passo));
  $("#posicao-modal").text(htmlCode[0].posicao);
  $("#rangeVal").val(htmlCode[0].largura);
  $("#rangeValLabel").text(htmlCode[0].largura);
  const percentual = htmlCode[0].largura + "%";
  $(".size-modal").css("max-width", percentual);

  rangeSize();
}

const btnHtmlCodeExcluir = document.querySelector("#btnHtmlCodeExcluir");

btnHtmlCodeExcluir.addEventListener("click", async () => {
  const selectElement = document.querySelector("#fluxos");
  await HtmlCodeService.excluirHtmlCodePorId(selectElement.value);
});

async function getHtmlCode(elemento, tabAtiva) {
  try {
    const htmlCode = await HtmlCodeService.obterHtmlCode(
      elemento.elementLocator.cssSelector,
      elemento.elementLocator.baseURI
    );
    if (htmlCode !== "") {
      $("#elemento").val(htmlCode.id);
      $("#summernote").summernote("code", htmlCode.code);
      $("#input-field-titulo").val(htmlCode.titulo);
      $("#ordenacao-modal").val(htmlCode.passo);
      $("#posicao-modal").text(htmlCode.posicao);
      $("#rangeVal").val(htmlCode.largura);
      $("#rangeValLabel").text(htmlCode.largura);
      const percentual = htmlCode.largura + "%";
      $(".size-modal").css("max-width", percentual);

      alvoSelecionado(elemento);
    } else {
      $("#elemento").val(0);
      $("#input-field-titulo").val("");
      $("#ordenacao-modal").text("");
      $("#posicao-modal").text("Baixo");
      $("#summernote").summernote("code", "");
      $("#rangeVal").val(80);
      $("#rangeValLabel").text(80);
      $(".size-modal").css("max-width", "80%");

      const element = tabAtiva.contentWindow.document.querySelector(
        elemento.elementLocator.cssSelector
      );

      if (
        element !== null &&
        element.type !== undefined &&
        (element.type === "select-one" || element.type === "text")
      ) {
        element.style.height = "25px";
      }
      //Arrumado element null
      if (element !== null) {
        debugger;

        html2canvas(element).then((canvas) => {
          let width = 0;
          let img = document.createElement("img");
          img.src = canvas.toDataURL("image/png");

          if (element.clientWidth > $(".modal-content").width()) {
            width = "100%";
          } else {
            width = element.clientWidth;
          }

          const textArea = printElement(img, width);

          $("#summernote").summernote("code", textArea);

          //     rangeSize();
        });
      }

      alvoSelecionado(elemento);
      marcaAlvo(elemento);
    }
  } catch (error) {
    console.error(error);
  }
}

function alvoSelecionado(elemento) {
  debugger;
  $("#elemento").val(elemento.elementLocator.cssSelector);
  $("#base-uri").val(elemento.elementLocator.baseURI);
  elementos.push(elemento.elementLocator);
}

async function onClickElement(tabAtiva, event) {
  if (isInspecionarPausado()) {
    if (event === undefined) event = window.event;

    let target = "target" in event ? event.target : event.srcElement;

    delete target.__jsaction;

    target.classList.remove("marked-element-temp");
    const elemento = new ElementsLocatorObj(
      location.href,
      null,
      new ElementLocator(
        getXPath(target),
        getCssPath(target),
        target.tagName,
        target.id,
        target.className,
        target.innerText,
        target.baseURI,
        getElementAttributes(target)
      )
    );

    openModal();
    summerNote();
    await getHtmlCode(elemento, tabAtiva);
    rangeSize();

    if (
      event.target.closest(".tabs") ===
      document
        .getElementsByClassName("yellow_pencil_iframe")
        .iframe.contentWindow.document.querySelector("#menu")
        .contentWindow.document.querySelector(".tabs")
    ) {
      // remove a classe "marked-element-temp" desse elemento clicado
      event.target.classList.remove("marked-element-temp");
    }
    event.target.classList.remove("marked-element-temp");
  }
}

$("#modalSummerNote").on("hidden.bs.modal", function (event) {
  inspecionarElemento();
});

function openModal() {
  var brnModal = document.querySelector(
    'button[data-target=".bd-example-modal-lg"]'
  );

  brnModal.click();
}

function tabMenuAtiva() {
  // const aba = getTabAtiva();

  let tabAtiva = -1;

  var iframe = document.getElementsByClassName("yellow_pencil_iframe")[0];

  if (iframe) {
    if (
      iframe.contentWindow &&
      iframe.contentWindow.document.querySelector("#menu")
    ) {
      var menu = iframe.contentWindow.document.querySelector("#menu");
      if (
        menu.contentWindow &&
        menu.contentWindow.document.querySelectorAll(".tab")
      ) {
        const tabsUl = menu.contentWindow.document.querySelector("ul.tabs");

        const activeTabLiElement = tabsUl.querySelector("li.active");

        if (activeTabLiElement.children[1]) {
          $("#pagina").val(
            activeTabLiElement.children[1].getAttribute("tooltip")
          );
        }

        tabAtiva =
          parseInt(activeTabLiElement.dataset.index) === 0
            ? ".tab"
            : `#tab-iframe-${activeTabLiElement.dataset.index}`;
      }
    }
  }

  if (tabAtiva === ".tab") {
    return document
      .getElementsByClassName("yellow_pencil_iframe")
      .iframe.contentWindow.document.querySelector("#menu");
  } else {
    const menu = document
      .getElementsByClassName("yellow_pencil_iframe")
      .iframe.contentWindow.document.querySelector("#menu");

    if (menu && menu.contentWindow) {
      return menu.contentWindow.document.querySelector(tabAtiva);
    }
  }
}

$("#iniciar-fluxo-help").on("click", function (event) {
  const btnGravar = document.getElementById("btn-gravar");
  const recognition = new webkitSpeechRecognition();

  recognition.lang = "pt-BR";
  recognition.continuous = true;
  recognition.interimResults = false;

  btnGravar.addEventListener("click", () => {
    recognition.start();
    console.log("A gravação foi iniciada");
  });

  recognition.onresult = (event) => {
    const resultado = event.results[0][0].transcript;
    console.log(`Você perguntou: ${resultado}`);
    // Aqui você pode fazer algo com a pergunta, como enviar para um servidor ou exibir na tela
  };

  recognition.onend = () => {
    console.log("A escuta foi encerrada");
  };

  if (
    event.target.innerText === "Iniciado..." ||
    event.target.innerText === "Pausado"
  ) {
    event.target.innerText = "Iniciar";
    document.getElementById("pausarInspencaoCheckbox").checked = false;
  } else {
    event.target.innerText = "Iniciado...";
    document.getElementById("pausarInspencaoCheckbox").checked = false;
    inspecionarElemento();
    obterPassosPorTabMenu($("#pagina").val());
  }
});

function inspecionarElemento() {
  let btnIniciar = document.getElementById("iniciar-fluxo-help");

  //Inicia IntroJJ
  //  startIntrJS();

  const tabAtiva = tabMenuAtiva();

  if (btnIniciar.innerText === "Iniciado...") {
    tabAtiva.contentWindow.document.addEventListener(
      "mouseover",
      function (event) {
        onMouseHoverElement(tabAtiva, event);
      }
    );

    tabAtiva.contentWindow.document.addEventListener(
      "mousedown",
      function (event) {
        onClickElement(tabAtiva, event);
      }
    );

    if (tabAtiva.getAttribute("id") !== "menu") {
      document
        .getElementsByClassName("yellow_pencil_iframe")
        .iframe.contentWindow.document.querySelector("#menu")
        .contentWindow.document.addEventListener("mouseover", function (event) {
          onMouseHoverElement(tabAtiva, event);
        });

      document
        .getElementsByClassName("yellow_pencil_iframe")
        .iframe.contentWindow.document.querySelector("#menu")
        .contentWindow.document.addEventListener("mousedown", function (event) {
          onClickElement(tabAtiva, event);
        });
    }
  }
}

async function obterPassosPorTabMenuIntroJs(tabMenu) {
  debugger;
  const htmlCodeTab = await HtmlCodeService.obterPassosPorTabMenu(tabMenu);

  let steps = [];
  if (htmlCodeTab !== "") {
    htmlCodeTab.forEach((htmlCode) => {
      const posicao = verificarPosicao(htmlCode.posicao);

      const larguraDoPai = window.innerWidth;
      const larguraEmPercentual = htmlCode.largura; //por exemplo
      const larguraEmPx = (larguraDoPai * larguraEmPercentual) / 100;

      const step = {
        element: tabMenuAtiva().contentDocument.querySelector(
          htmlCode.elemento
        ),
        intro: htmlCode.code,
        width: larguraEmPx + "px",
        position: posicao,
      };

      steps.push(step);
    });
  }

  return steps;
}

function verificarPosicao(posicao) {
  switch (posicao) {
    case "Cima":
      return "top";
    case "Baixo":
      return "bottom";
    case "Direita":
      return "right";
    case "Esquerda":
      return "left";
    default:
      return "bottom";
  }
}

$(".introjs-nextbutton").on("click", function () {
  // Espera a animação do elemento da etapa anterior terminar
  setTimeout(function () {
    var tooltip = $(".introjs-tooltip");
    var helperLayer = $(".introjs-helperLayer");

    // Calcula a posição da tooltip em relação ao helperLayer
    var tooltipLeft =
      helperLayer.offset().left + helperLayer.width() / 2 - tooltip.width() / 2;
    var tooltipTop = helperLayer.offset().top - tooltip.height() - 10;

    tooltip.css({
      left: tooltipLeft + "px",
      top: tooltipTop + "px",
    });
  }, 500); // Tempo de espera em milissegundos
});

const btnHtmlCodeExibir = document.querySelector("#btn-exibir");
btnHtmlCodeExibir.addEventListener("click", async () => {
  const stepsHtmlCodeTab = await obterPassosPorTabMenuIntroJs(
    $("#pagina").val()
  );

  startIntroJS(stepsHtmlCodeTab);
});

function ajustarPosicaoIntro1() {
  $(".introjs-helperLayer").css(
    "top",
    parseInt($(".introjs-helperLayer").css("top")) - 31 + "px"
  );
  $(".introjs-helperLayer").css(
    "left",
    parseInt($(".introjs-helperLayer").css("left")) - 45 + "px"
  );
}

function ajustarPosicaoIntroMenor() {
  $(".introjs-helperLayer").css(
    "top",
    parseInt($(".introjs-helperLayer").css("top")) + 31 + "px"
  );
  $(".introjs-helperLayer").css(
    "left",
    parseInt($(".introjs-helperLayer").css("left")) + 45 + "px"
  );
}

var helperLayerTop = null; // Variável global para armazenar a posição vertical original da camada de ajuda
function ajustarPosicaoIntro() {
  var currentStep = intro._introItems[intro._currentStep];
  var targetElement = currentStep.element;

  // volta a posição original
  $(".introjs-helperLayer").css({
    top: "",
    left: "",
  });

  $(".introjs-tooltip").css({
    top: "",
    left: "",
  });

  if (targetElement) {
    var $target = $(targetElement);

    // obtém a posição do elemento mapeado
    var targetPos = $target.offset();

    // calcula a posição correta do helper
    var helperTop, helperLeft;

    if (currentStep.step === 1) {
      //    para o primeiro elemento, ajuste para a posição abaixo do elemento
      helperTop = targetPos.top - 10; // ajuste de margem
      helperLeft =
        targetPos.left +
        $target.outerWidth() / 2 -
        $(".introjs-helperLayer").outerWidth() / 2;
      $(".introjs-tooltip").css({
        top: "50px",
        left: "-40px",
      });
    } else {
      // para os demais elementos, ajuste para a posição acima do elemento
      helperTop = targetPos.top - 10; // ajuste de margem
      helperLeft =
        targetPos.left +
        $target.outerWidth() / 2 -
        $(".introjs-helperLayer").outerWidth() / 2;
      $(".introjs-tooltip").css({
        top: "",
        left: "",
      });
    }

    // define a nova posição
    $(".introjs-helperLayer").css({
      top: helperTop,
      left: helperLeft,
    });

    if (currentStep.step === 1) {
      $(".introjs-tooltip").css({
        top: "50px",
        left: "-40px",
      });
    } else {
      $(".introjs-tooltip").css({
        top: "0px",
        left: "-40px",
      });
    }
  }
}

function startIntroJS(stepsHtmlCodeTab) {
  intro.setOptions({
    doneLabel: "Fim",
    nextLabel: "Proximo &rarr;",
    prevLabel: "&larr; Voltar",
    tooltipPosition: "bottom",
    exitOnEsc: true,
    showStepNumbers: true,
    keyboardNavigation: true,
    scrollToElement: false,
    showButtons: true,
    stepNumbersOfLabel: "/",
    overlayOpacity: 0.6,
    positionPrecedence: ["bottom", "top", "right", "left"],
    // oncomplete: function () {
    //   console.log("oncomplete triggered");
    //   ajustarPosicaoIntro();
    // },

    steps: stepsHtmlCodeTab,
    // steps: [
    //   {
    //     element:
    //       tabMenuAtiva().contentDocument.querySelector("#tab-gerada > a"),
    //     intro:
    //       'Bem Vindo ao <span style="color: #007bff"><b>Bluesoft-Time</b></span>, ao clicar sobre o botão <i>"Proximo"</i> iremos te guiar pelo site',
    //     width: "800px",
    //     position: "rigth",
    //   },

    //   {
    //     element: document
    //       .getElementById("iframe")
    //       .contentWindow.document.querySelector("#tab-gerada > a"),
    //     intro:
    //       'Bem Vindo ao <span style="color: #007bff"><b>Bluesoft-Time</b></span>, ao clicar sobre o botão <i>"Proximo"</i> iremos te guiar pelo site',
    //     width: "800px",
    //   },
    //   {
    //     intro:
    //       '<p><br></p><img src="data:image/png;base64," <="" img="">',
    //     width: "800px",
    //   },

    //   {
    //     intro:
    //       '<p><br></p><p><img src="data:image/png;base64," style="width: 744px;" data-filename="image.png"></p>',
    //     width: "800px",
    //   },

    //   {
    //     element: "#avatares-equipe",
    //     intro:
    //       '<h4  data-step="1" data-title="asd" style="text-align: center">Avatares</h4> </br>' +
    //       '<span style="color: #007bff"><b>Área Avatar:</b></span> É exibido os usuários que pertence a Squad selecionada' +
    //       "<p><b>:Exemplo</b></p>" +
    //       "Caso selecionado a Squad “ERP Truck”, é exibido os colaboradores <i>Alexandre, Hugo, Lucas, Patricia e Wesley<i>",
    //     position: "right",
    //   },
    //   {
    //     element: ".gLFyf",
    //     intro:
    //       '<h4 style="text-align: center">Categorias</h4> </br>' +
    //       '<table style="position: relative; right: 116px;">' +
    //       "<tbody>" +
    //       "<tr>" +
    //       "<td>Atividade Jurítica&nbsp;</td>" +
    //       '<td><div class="circle-tipos LILAS"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Atividade em Análise&nbsp;</td>" +
    //       '<td><div class="circle-tipos" style="background-color: #da4453;"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Auditoria ERP&nbsp;</td>" +
    //       '<td><div class="circle-tipos AMARELO"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Auditoria de Uso do ERP&nbsp;</td>" +
    //       '<td><div class="circle-tipos VERDE_PRIMAVERA></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Ação&nbsp;</td>" +
    //       '<td><div class="circle-tipos VERMELHO"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Banco de Dados&nbsp;</td>" +
    //       '<td><div class="circle-tipos LARANJA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Bug Homologação&nbsp;</td>" +
    //       '<td><div class="circle-tipos AZUL_ARDOSIA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Bug Pôs Homologação&nbsp;</td>" +
    //       '<td><div class="circle-tipos LAVANDA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Bug Urgente&nbsp;</td>" +
    //       '<td><div class="circle-tipos ROSA_CHOQUE"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Bug Não Urgente&nbsp;</td>" +
    //       '<td><div class="circle-tipos ROSA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Casos de Teste&nbsp;</td>" +
    //       '<td><div class="circle-tipos VERDE_AGUA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Documentação&nbsp;</td>" +
    //       '<td><div class="circle-tipos CINZA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Divída Tecnica&nbsp;</td>" +
    //       '<td><div class="circle-tipos AZUL"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Erro de Log&nbsp;</td>" +
    //       '<td><div class="circle-tipos SALMAO"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Força de Vendas&nbsp;</td>" +
    //       '<td><div class="circle-tipos OLIVA"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Grooming&nbsp;</td>" +
    //       '<td><div class="circle-tipos AZUL_CADETE"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Melhoria&nbsp;</td>" +
    //       '<td><div class="circle-tipos CIANO_ESCURO"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>Otimização&nbsp;</td>" +
    //       '<td><div class="circle-tipos BRANCO"></div></td>' +
    //       "</tr>" +
    //       "<tr>" +
    //       "<td>&nbsp;</td>" +
    //       "<td>&nbsp;</td>" +
    //       "</tr>" +
    //       "</tbody>" +
    //       "</table>" +
    //       "Ao clicar sobre algum tipo é filtrado apenas o tipo do ticket do qual foi selecionado",
    //     position: "left",
    //   },
    //   {
    //     element: "#multisquad",
    //     intro:
    //       '<h4 style="text-align: center">Multi Squad</h4> </br>' +
    //       'Ao Marcar a opção <span style="color: #007bff"><b>Multi-Squad:</b></span> é exibido no painel frontal do kaban os tickets no qual uma determinada equipe ficou encarregada de desenvolver, ou seja é ilustrado o ticket que outra Squad está desenvolvendo para ajudar outra equipe' +
    //       "<h3>:Exemplo de Ticket Multi-Squad</h3>" +
    //       "<center>" +
    //       '<div class="box ticket" cor="CIANO_ESCURO">' +
    //       '<div class="badge badge-primary badge-clientes" title="Bluesoft">Bluesoft</div>' +
    //       '<p class="text-center">' +
    //       "<b>#246515 (To Test)</b></p>" +
    //       '<div class="text-center">' +
    //       "</div>" +
    //       '<p class="text-center" title="Relatório Consulta de Notas Fiscais Recebidas">' +
    //       "Relatório Consulta de Notas Fiscais Recebidas" +
    //       "</p><p><small>" +
    //       "Total: 3:01" +
    //       "|" +
    //       "Etapa: 124h" +
    //       "|" +
    //       "LT: 0d" +
    //       "</small>" +
    //       "</p>" +
    //       "<p>" +
    //       '</p><div class="multi-squad">Multi Squad</div>' +
    //       '<div class="badge badge-danger" title="Squads">ERP Nitro</div>' +
    //       "<p></p>" +
    //       "</div>" +
    //       "</center>",
    //     position: "left",
    //     width: "800px",
    //   },
    //   {
    //     element: tabMenuAtiva().contentDocument.querySelector(
    //       "#wms-gerenciamento-collapse > bs-form-input:nth-child(3)"
    //     ),
    //     intro:
    //       '<h4 style="text-align: center">Multi Squad</h4> </br>' +
    //       "<b>?O que é Squad</b>" +
    //       '<div><span style="color: #007bff"><b>Squads:</b></span> São pequenas equipes multidisciplinares encarregadas de desenvolver projetos de forma autônoma, o que favorece uma comunicação eficaz, mais direta e sem ruídos</div>' +
    //       "Ao selecionar uma Squad é listado os tickets no Kanban responsavel pela equipe",
    //     position: "right",
    //     width: "800px",
    //   },
    //   {
    //     element: "#wms-gerenciamento-collapse > bs-form-input:nth-child(3)",
    //     intro:
    //       '<h4 style="text-align: center">Gráficos</h4> </br>' +
    //       "<table>" +
    //       "<tbody>" +
    //       '<tr style="height: 23px;">' +
    //       '<td rowspan="4">&nbsp;<img class="ajuda-grafico"></td>' +
    //       "<td ><b>Bugs:</b> Exibi a quantidade de bugs&nbsp</td>" +
    //       "</tr>" +
    //       "<tr >" +
    //       "<td ></br>  <b>Progesso das Metas:</b> Exibi a evolu&ccedil;&atilde;o das metas em percentual </td>" +
    //       "</tr>" +
    //       "<tr >" +
    //       "<td ><b>Progesso das Metas por Tarefa:</b> Exibi a evolu&ccedil;&atilde;o das tarefas associada as metas em percentual</td>" +
    //       "</tr>" +
    //       "<tr >" +
    //       "<td ><b>LeadTime:</b> Exibi o tempo decorrido que cada ticket levou</td>" +
    //       "</tr>" +
    //       "</tbody>" +
    //       "</table>",
    //     position: "left",
    //   },
    // ],
  });

  intro.onchange(function (targetElement) {
    var stepNumber = 0;
    var width = "350px";

    for (var i = 1; i < intro._options.steps.length; i++) {
      if (intro._options.steps[i].element == targetElement) {
        if ($("#pagina").val() === "") {
          setTimeout(function () {
            var tooltip = $(".introjs-tooltip");
            var helperLayer = $(".introjs-helperLayer");

            // Calcula a posição da tooltip em relação ao helperLayer

            var tooltipTop = 50;

            var tooltipLeft = -40;

            tooltip.css({
              left: tooltipLeft + "px",
              top: tooltipTop + "px",
            });
          }, 500); // Tempo de espera em milissegundos
        }
        stepNumber = i;
        width = intro._options.steps[i].width;
        $(".introjs-tooltip").css("min-width", width);
        // //  ajustarPosicaoIntro();
        // if (
        //   intro._options.steps[i].element.className ===
        //     "menu-item ng-scope" ||
        //   intro._options.steps[i].element.className ===
        //     "menu-item ng-scope introjs-showElement"
        // ) {
        //   menu = true;
        //   $(".introjs-helperLayer").css(
        //     "top",
        //     parseInt($(".introjs-helperLayer").css("top")) -
        //       31 +
        //       "px!important"
        //   );
        //   $(".introjs-helperLayer").css(
        //     "left",
        //     parseInt($(".introjs-helperLayer").css("left")) -
        //       45 +
        //       "px!important"
        //   );
        // }
        //   break;
      }
    }
    // switch (stepNumber) {
    //   case stepNumber:
    //     $(".introjs-tooltip").css("min-width", width);
    //     intro.refresh();
    //     break;
    // }
  });

  intro.onafterchange(function (targetElement) {
    if ($("#pagina").val() === "") {
      ajustarPosicaoIntro();
    }
  });

  // intro.onafterchange(function (targetElement) {
  //   intro.refresh();
  //   ajustarPosicaoIntro();
  // });

  intro.onbeforechange(function (targetElement) {
    if ($("#pagina").val() === "") {
      ajustarPosicaoIntro();
    }
  });

  intro.start();

  // intro.oncomplete(function () {
  //   alert("Fim");
  // });

  intro.refresh();
}

const summerNoteTemplate = function (context) {
  let ui = $.summernote.ui;
  let button = ui.button({
    contents: '<i class="fa fa-pencil-square-o fa-2"/> Padrão',
    tooltip: "Padrao",
    click: function () {
      //Apicar template pronto
      context.invoke("editor.insertText", "Borda Ativa");
    },
  });
  return button.render();
};

let EventData = function (context) {
  let ui = $.summernote.ui;
  let event = ui.buttonGroup([
    ui.button({
      contents: "Alinhar Modal",
      data: {
        toggle: "dropdown",
      },
    }),
    ui.dropdown({
      items: ["Esquerda", "Direita", "Cima", "Baixo"],
      click: function (event) {
        let value = $(event.target).text();
        switch (value) {
          case "Esquerda":
            $("#posicao-modal")[0].innerText = "Esquerda";
            context.invoke("editor.insertText", "left");

            break;
          case "Direita":
            $("#posicao-modal")[0].innerText = "Direita";
            context.invoke("editor.insertText", "right");

            break;
          case "Cima":
            $("#posicao-modal")[0].innerText = "Cima";
            context.invoke("editor.insertText", "topa");

            break;
          case "Baixo":
            $("#posicao-modal")[0].innerText = "Baixo";
            context.invoke("editor.insertText", "bottom");

            break;
        }
      },
    }),
  ]);

  return event.render();
};

function summerNote() {
  $("#summernote").summernote({
    lang: "pt-BR",
    placeholder: "Hello stand alone ui",
    tabsize: 2,
    height: 120,
    toolbar: [
      ["style", ["style"]],
      ["font", ["bold", "underline", "clear"]],
      ["color", ["color"]],
      ["para", ["ul", "ol", "paragraph"]],
      ["table", ["table"]],
      ["insert", ["link", "picture", "video"]],
      ["view", ["fullscreen", "codeview", "help"]],
      ["mybutton", ["padrao", "posicao"]],
      ["eventButton", ["event"]],
    ],
    buttons: {
      padrao: summerNoteTemplate,
      event: EventData,
    },
  });
}

function fecharModalImagem() {
  $(".modal .note-modal").modal("hide");
  inspecionarElemento();
}

$("#markAllCheckbox").click(function () {
  checkElements();
});

$("#pausarInspencaoCheckbox").click(function () {
  pausar();
});

var elementLocatorOnOff = !1;
var markAllElementsOnOff = !1;
var currentLocationHref;
var currentPageElements = [];
var currentCheckElements = [];
var contextMenuID;
var currentElement;
var contentDocument;

var objSelecionado = null;
var mouseOffset = null;
function addEvent(obj, evType, fn) {
  if (typeof obj == "string") {
    if (null == (obj = document.getElementById(obj))) {
      throw new Error(
        "Elemento HTML não encontrado. Não foi possível adicionar o evento."
      );
    }
  }
  if (obj.attachEvent) {
    return obj.attachEvent("on" + evType, fn);
  } else if (obj.addEventListener) {
    return obj.addEventListener(evType, fn, true);
  } else {
    throw new Error(
      "Seu browser não suporta adição de eventos. Senta, chora e pega um navegador mais recente."
    );
  }
}
function mouseCoords(ev) {
  if (typeof ev.pageX !== "undefined") {
    return { x: ev.pageX, y: ev.pageY };
  } else {
    return {
      x: ev.clientX + document.body.scrollLeft - document.body.clientLeft,
      y: ev.clientY + document.body.scrollTop - document.body.clientTop,
    };
  }
}
function getPosition(e, ev) {
  var ev = ev || window.event;
  if (e.constructor == String) {
    e = document.getElementById(e);
  }
  var left = 0,
    top = 0;
  var coords = mouseCoords(ev);

  while (e.offsetParent) {
    left += e.offsetLeft;
    top += e.offsetTop;
    e = e.offsetParent;
  }
  left += e.offsetLeft;
  top += e.offsetTop;
  return { x: coords.x - left, y: coords.y - top };
}

function dragdrop(local_click, caixa_movida) {
  if (local_click.constructor == String) {
    local_click = document.getElementById(local_click);
  }
  if (caixa_movida.constructor == String) {
    caixa_movida = document.getElementById(caixa_movida);
  }

  local_click.style.cursor = "move";
  if (!caixa_movida.style.position || caixa_movida.style.position == "static") {
    caixa_movida.style.position = "relative";
  }
  local_click.onmousedown = function (ev) {
    objSelecionado = caixa_movida;
    mouseOffset = getPosition(objSelecionado, ev);
    if (mouseOffset < 0) {
      objSelecionado.style.margin = "50px";
    }
  };
  document.onmouseup = function () {
    objSelecionado = null;
  };
  document.onmousemove = function (ev) {
    if (objSelecionado) {
      var ev = ev || window.event;
      var mousePos = mouseCoords(ev);
      var pai = objSelecionado.parentNode;
      var w = mousePos.x - mouseOffset.x - pai.offsetLeft;
      var h = mousePos.y - mouseOffset.y - pai.offsetTop;
      var areaHorizontal =
        document.body.clientWidth - objSelecionado.clientWidth - 2;
      var areaVertical =
        document.body.clientHeight - objSelecionado.clientHeight - 2;

      objSelecionado.style.left = w + "px";
      objSelecionado.style.top = h + "px";

      if (w >= areaHorizontal) {
        objSelecionado.style.left = areaHorizontal;
      }
      if (w <= 0) {
        objSelecionado.style.left = 0;
      }
      if (h >= areaVertical) {
        objSelecionado.style.top = areaVertical;
      }
      if (h <= 0) {
        objSelecionado.style.top = 0;
      }

      objSelecionado.style.margin = "0px";
      return false;
    }
  };
}

dragdrop("movedor", "movido");

var fixo = 1;
$(".wyp-save-btn").click(function () {
  fixarBarraFerramentaDireita();
});

const fixarBarraFerramentaDireita = async () => {
  $(".bl-bluesoft-help").toggleClass("wyp-fix-pan");

  if (fixo === 1) {
    fixo = 0;
    var iframe = $("#iframe").width() - $("#movido").width();
    $("#iframe").css("width", iframe);
    $("#movido").height(document.getElementById("iframe").offsetHeight - 48);
    $("#w-maximizar-painel")[0].className = "fa fa-window-restore fa-1x";
  } else {
    var iframe = $("#iframe").width() + $("#movido").width();
    $("#iframe").css("width", iframe);
    $("#movido").height("auto");
    $("#w-maximizar-painel")[0].className = "fa fa-window-maximize fa-1x";
    fixo = 1;
  }
};

window.onresize = function (event) {
  if (fixo === 0) {
    var a = window.innerWidth - $("#movido").width();
    $("#iframe").css("width", a);
    $("#movido").height(document.getElementById("iframe").offsetHeight - 48);
  } else {
    $("#iframe").css("width", screen.width);
    $("#movido").height("auto");
  }
};

$("#rangeVal").change(function (newVal) {
  const percentual = newVal.currentTarget.value + "%";
  $(".size-modal").css("max-width", percentual);
});

function ajustarLayout(element) {
  const a = $(".introjs-helperLayer").position();
  a.top = a.top + 33;
  a.left = a.left + 45;
  $(".introjs-helperLayer").css("top", a.top + "px");
  $(".introjs-helperLayer").css("left", a.left + "px");

  const b = $(".introjs-tooltipReferenceLayer").position();
  b.top = b.top + 33;
  b.left = b.left + 45;
  $(".introjs-tooltipReferenceLayer").css("top", b.top + "px");
  $(".introjs-tooltipReferenceLayer").css("left", b.left + "px");

  const c = $(".introjs-tooltip").position();
  c.top = c.top + 3;

  $(".introjs-tooltip").css("top", c.top + "px");
}

function marcaTodosElementosSelecionados() {
  return document.getElementById("markAllCheckbox").checked;
}

function ElementLocator(
  xpath,
  cssSelector,
  tagName,
  id,
  className,
  innerText,
  baseURI,
  attributes
) {
  this.xpath = xpath;
  this.cssSelector = cssSelector;
  this.tagName = tagName;
  this.id = id;
  this.className = className;
  this.innerText = innerText;
  this.baseURI = baseURI;
  this.attributes = attributes;
}
function ElementsLocatorObj(
  locationHref,
  turnOnOff,
  elementLocator,
  checkElements
) {
  this.turnOnOff = turnOnOff;
  this.locationHref = locationHref;
  this.elementLocator = elementLocator;
  this.checkElements = checkElements;
}
function sendRunTimeMessage(message) {
  chrome.tabs.query({ active: !0, currentWindow: !0 }, function (tabs) {
    if (tabs[0]) {
      chrome.tabs.sendMessage(tabs[0].id, message);
    }
  });
}
function ElementAttribute(name, value, updateDate) {
  this.name = name;
  this.value = value;
  this.updateDate = updateDate;
}
function CheckElement(
  elementLocator,
  isExist,
  unMachAttributes,
  xpathIsDuplicated,
  cssSelectorIsDuplicated
) {
  this.elementLocator = elementLocator;
  this.isExist = isExist;
  this.unMachAttributes = unMachAttributes;
  this.xpathIsDuplicated = xpathIsDuplicated;
  this.cssSelectorIsDuplicated = cssSelectorIsDuplicated;
}

function rangeSize() {
  const footer = $(
    "body > div.modal.fade.bd-example-modal-lg.show > div > div > div.modal-footer"
  );

  const range = document.getElementById("rangeVal");
  range.setAttribute("step", "1");
  range.setAttribute("type", "range");
  range.setAttribute("min", "35");
  range.setAttribute("max", "100");
  range.setAttribute("value", "100");
  range.setAttribute(
    "oninput",
    "document.getElementById('rangeValLabel').innerHTML = this.value + document.getElementById('percentual').innerText;"
  );
  range.setAttribute(
    "onchange",
    "document.getElementsByClassName('size-modal')[0].style.width = this.value + document.getElementById('percentual').innerText;"
  );

  const pDescricao = document.querySelector(".p-size-summer");
  const percentualAtual = document.getElementById("percentual");
  if (percentualAtual == null) {
    var percentual = document.createElement("span");
    percentual.setAttribute("id", "percentual");
    percentual.innerText = "%";
    pDescricao.append(percentual);
    pDescricao.appendChild(range);
  }

  const rangeValLabel = document.querySelector("#rangeValLabel");
  if (rangeValLabel == null) {
    var size = document.createElement("label");
    size.setAttribute("id", "rangeValLabel");
    size.setAttribute("for", "rangeVal");
    size.setAttribute("class", "p-result-summer");
    size.innerText = "100%";
    pDescricao.append(size);
  }

  footer.append(pDescricao);
  // }
}

function addImageBorda(img, width) {
  img.setAttribute("borderColor", "red");
  img.setAttribute("border-style", "solid");
  img.setAttribute("border-width", "2px");
  img.setAttribute("widht", width);
}

var currentElement;

function borderElement() {
  return document.getElementById("bordaElementoSelecionado").value == "true"
    ? ";border-style:solid;border-color:red"
    : "";
}

function printElement(img, width) {
  return document.getElementById("printElementoSelecionado").value == "true"
    ? "<img src='" +
        img.src +
        "' " +
        "style=width:" +
        width +
        borderElement() +
        "> </img>"
    : "";
}

const showVal = (newVal) => {
  const percentual = newVal + "%";
};

function positionXY(x) {
  var o = document
    .getElementById("iframe")
    .contentWindow.document.querySelector(x);
  var l = o.offsetLeft;
  var t = o.offsetTop;
  while ((o = o.offsetParent)) l += o.offsetLeft;
  o = tabMenuAtiva().contentWindow.document.querySelector(x);
  while ((o = o.offsetParent)) t += o.offsetTop;
  return [l, t];
}

function getAbsPosition(element) {
  var rect = element.getBoundingClientRect();
  return { x: rect.left, y: rect.top };
}

function checkVisible(elm) {
  if (elm) {
    var rect = elm.getBoundingClientRect();
    var viewHeight = Math.max(
      document.documentElement.clientHeight,
      window.innerHeight
    );
    return !(rect.bottom < 0 || rect.top - viewHeight >= 0);
  }
}

function elementoSelecionado(element) {
  if (element) {
    $("#elemento-point")[0].innerText =
      element.tagName + " > " + element.innerText.split("\n")[0].trim();
  }
}

function markElement(element) {
  if (element) {
    element.classList.add("marked-element");
  }
}
function markElementTemp(element) {
  debugger;
  if (element) {
    element.classList.add("marked-element-temp");
  }
}
function markElementSuccess(element) {
  if (element) {
    element.classList.add("mark-successfully");
  }
}

function unMarkElement(element, isTemp) {
  if (element) {
    if (isTemp) {
      element.classList.remove("mark-successfully");
      element.classList.remove("marked-element-temp");
    } else {
      element.classList.remove("marked-element");
      element.classList.remove("marked-element-temp");
    }
    if (element.getAttribute("class") == "") {
      element.removeAttribute("class");
    } else {
      element.setAttribute("class", element.getAttribute("class"));
    }
  }
}

function unMarkAllMarks(tabAtiva) {
  var markElements = tabAtiva.contentWindow.document.querySelectorAll(
    '*[class*="marked-element"]'
  );
  if (markElements.length > 0) {
    // var markElement = tabAtiva.contentWindow.document.querySelectorAll(
    //   '*[class*="marked-element"]'
    // );

    // unMarkElement(markElement, true);

    for (var index = 0; index < markElements.length; index++) {
      unMarkElement(markElements[index]);
    }

    var markElementsTemp = tabAtiva.contentWindow.document.querySelectorAll(
      '*[class*="marked-element-temp"]'
    );

    for (var index = 0; index < markElementsTemp.length; index++) {
      unMarkElement(markElementsTemp[index], !0);
    }

    var markElementsSuccess = tabAtiva.contentWindow.document.querySelectorAll(
      '*[class*="mark-successfully"]'
    );
    for (var index = 0; index < markElementsSuccess.length; index++) {
      unMarkElement(markElementsSuccess[index], !0);
    }
  }
}

function showElementOnPage(elementLocator) {
  // unMarkAllTempElements();
  if (currentElement) {
    markElementSuccess(currentElement);
    currentElement = null;
  } else {
    var element = tabMenuAtiva().contentWindow.document.querySelector(
      elementLocator.cssSelector
    );
    if (element) {
      markElementTemp(element);
      if (!checkVisible(element)) {
        element.scrollIntoView({
          behavior: "smooth",
          block: "center",
          inline: "center",
        });
      }
    }
  }
}

function pausar() {
  let pausado = document.getElementById("pausarInspencaoCheckbox").checked;
  let btnIniciar = document.getElementById("iniciar-fluxo-help");

  if (pausado) {
    btnIniciar.innerText = "Pausado";
  } else {
    btnIniciar.innerText = "Iniciado...";
    inspecionarElemento();
  }
}

function checkElements() {
  const tabAtiva = tabMenuAtiva();

  if (marcaTodosElementosSelecionados()) {
    getElements(tabAtiva);
  } else {
    unMarkAllMarks(tabAtiva);
  }
}
function getElements(tabAtiva) {
  currentCheckElements = null;
  currentPageElements = elementos ? elementos : [];

  if (currentPageElements) {
    for (var i = 0; i < currentPageElements.length; i++) {
      markElement(
        tabAtiva.contentWindow.document.querySelector(
          currentPageElements[i].cssSelector
        )
      );
    }
    currentElement = null;
  }
}

$(document).ready(function () {
  $(".draggable").mousedown(function (e) {
    var posX = e.clientX;
    var posY = e.clientY;
    var divTop = $(this).offset().top;
    var divLeft = $(this).offset().left;
    var offsetX = posX - divLeft;
    var offsetY = posY - divTop;

    $(document).mousemove(function (e) {
      var newPosX = e.clientX - offsetX;
      var newPosY = e.clientY - offsetY;
      $(".draggable").offset({ top: newPosY, left: newPosX });
    });
  });

  $(document).mouseup(function () {
    $(document).unbind("mousemove");
  });
});

// // selecionando a label pela ID
// const label = document.querySelector("#balao-fala");

// // array com os textos que irão se alternar
// const texts = [
//   "Olá eu sou Genie",
//   "Eu sou um gênio da programação",
//   "Posso te ajudar com qualquer dúvida em Javascript",
// ];

// let i = 0;
// // definindo a ação a ser executada a cada 10 segundos
// setInterval(() => {
//   // verificando se chegou ao fim do array de textos
//   if (i >= texts.length) {
//     // reiniciando o contador para exibir o primeiro texto novamente
//     i = 0;
//   }
//   // adicionando conteúdo dinâmico à label
//   label.textContent = texts[i];
//   // incrementando o contador para exibir o próximo texto na próxima execução
//   i++;
// }, 10000); // tempo em milissegundos para cada execução (10 segundos)
const formPergunta = document.getElementById("form-pergunta");

formPergunta.addEventListener("submit", async (event) => {
  event.preventDefault(); // previne o comportamento padrão do formulário

  const inputPergunta = document.getElementById("input-pergunta");
  let pergunta = inputPergunta.value;

  // Adiciona o loading
  const loading = document.getElementById("loading");
  loading.style.display = "block";

  const response = await fetch("https://api.openai.com/v1/completions", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization:
        "Bearer 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",
    },
    body: JSON.stringify({
      model: "text-davinci-003",
      temperature: 0.8,
      max_tokens: 500,
      n: 1,
      stop: null,
      prompt:
        "quais são as informações disponíveis sobre " +
        pergunta +
        " site:ajuda.bluesoft.com.br",

      // output_filters: {
      //   property_filter: "https://ajuda.bluesoft.com.br",
      // },
    }),
  });
  const data = await response.json();
  console.log(response.status); // deve imprimir "200"

  // Remove o loading assim que os dados são carregados
  loading.style.display = "none";
  // manipular a resposta da API como desejar, por exemplo:
  // const resultadoPergunta = document.getElementById("resultado-pergunta");
  //resultadoPergunta.innerHTML =;
  const utterance = new SpeechSynthesisUtterance(data.choices[0].text);
  utterance.rate = 1.4;
  speechSynthesis.speak(utterance);

  var intro = introJs();
  intro.setOptions({
    minWidth: 400,
    doneLabel: "Fim",
    nextLabel: "Próximo &rarr;",
    prevLabel: "&larr; Voltar",
    tooltipPosition: "bottom",
    exitOnEsc: true,
    showStepNumbers: true,
    keyboardNavigation: true,
    scrollToElement: false,
    showButtons: true,
    stepNumbersOfLabel: "/",
    overlayOpacity: 0.6,
    positionPrecedence: ["bottom", "top", "right", "left"],
    steps: [
      {
        intro:
          "<span style='color: #007bff'>" +
          pergunta
            .split(" ")
            .map(
              (palavra) => palavra.charAt(0).toUpperCase() + palavra.slice(1)
            )
            .join(" ") +
          "</b></span></br>" +
          data.choices[0].text,
      },
    ],
  });
  var customCSS = document.createElement("style");
  customCSS.innerHTML =
    ".introjs-tooltip { width: 800px !important; min-width: 800px !important}";
  document.head.appendChild(customCSS);
  intro.start();

  $(".introjs-donebutton").on("click", function () {
    setTimeout(() => {
      speechSynthesis.cancel();
    }, 1000); // Cancela a voz após 5 segundos
    intro.exit();
  });
});

const DragDrop = {
  fixarBarraFerramentaDireita,
};
