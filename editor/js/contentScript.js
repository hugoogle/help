...
style.type = "text/css";
style.href = chrome.extension.getURL("css/general.css");
...
function onClickElement(event) {
  ...
  chrome.runtime.sendMessage(elo);
  currentElement = t;
}
...
function unMarkAllTempElements() {
  unMarkElements(
    document.querySelectorAll('*[class*="marked-element-temp"]'),
    !0
  );
  unMarkElements(
    document.querySelectorAll('*[class*="mark-successfully"]'),
    !0
  );
}
function unMarkAllMarks() {
  var markElements = document.querySelectorAll('*[class*="marked-element"]');
  for (var index = 0; index < markElements.length; index++) {
    unMarkElement(markElements[index]);
  }
  var markElementsTemp = document.querySelectorAll(
    '*[class*="marked-element-temp"]'
  );
  for (var index = 0; index < markElementsTemp.length; index++) {
    unMarkElement(markElementsTemp[index], !0);
  }
  var markElementsSuccess = document.querySelectorAll(
...
chrome.runtime.onMessage.addListener(function (eventPageMessage) {
  ...
  if (eventPageMessage.elementLocator) {
    if (eventPageMessage.elementLocator.cssSelector) {
      showElementOnPage(eventPageMessage);
    } else if (eventPageMessage.elementLocator.length) {
      if (currentElement) {
        markElementSuccess(currentElement);
        currentElement = null;
      }
      for (var i = 0; i < eventPageMessage.elementLocator.length; i++) {
        markElement(
          document.querySelector(eventPageMessage.elementLocator[i].cssSelector)
        );
      }
    }
  } else {
  ...
  if (eventPageMessage.checkElements) {
    getElementsCheckResults(eventPageMessage.checkElements);
  }
});
function getElementsCheckResults(checkElements) {
  ...
  chrome.runtime.sendMessage(
    new ElementsLocatorObj(location.href, null, null, checkElements)
  );
}
function showElementOnPage(eventPageMessage) {
  unMarkAllTempElements();
  if (currentElement) {
    markElementSuccess(currentElement);
    currentElement = null;
    ...
    var element = document.querySelector(
      eventPageMessage.elementLocator.cssSelector
    );
    if (element) {
...
function sendElementsLocatorObj() {
  if (document.visibilityState == "visible") {
    chrome.runtime.sendMessage(
      new ElementsLocatorObj(location.href, null, null)
    );
  }
}
document.addEventListener("visibilitychange", function () {
  sendElementsLocatorObj();
});
sendElementsLocatorObj();
onClickElement("a");
